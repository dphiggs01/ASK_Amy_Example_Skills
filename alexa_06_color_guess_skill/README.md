# The Alexa Color Guess Skill
This example skill demonstrates simple session persistence by asking you
...
1. Update ``arn:aws:iam::**********:role/alexa_lambda_role`` in cli_config.json.
    ...
    *  ~~~
        $ cd xxxxxxx/ask_amy_example_skills/alexa_06_color_guess_skill
        $ vim cli_config.json
        ~~~
...
3. Create a new skill in your Amazon Development Account.
    ...
    * Enter a _Skill Name_ (color guess) and select _custom_ as the skill type and _Provision your own_ as the method to host the backend. Then select `Create a skill`.
...
~~~
$ ask-amy-cli logs --log-group-name /aws/lambda/alexa_color_guess_skill
~~~
