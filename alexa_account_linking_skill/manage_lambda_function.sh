...
# You also must set the ASK_AMY_HOME if you intent to use the DEV
# option
...
function copy_local_ask_amy {
    if [[ "$ASK_AMY_HOME" == "" ]]; then
        echo ASK_AMY_HOME environment variable NOT set!!!
        echo This is required for use of local copy of ask_amy
        exit -1
    else
        echo "Copying local version of ask_amy"
        rm -rf  $SKILL_DIR/dist/ask_amy
   	    cp -r  $ASK_AMY_HOME $SKILL_DIR/dist
   	fi
}
