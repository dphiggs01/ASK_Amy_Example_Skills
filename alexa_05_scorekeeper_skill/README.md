...
1. Update ``arn:aws:iam::**********:role/alexa_lambda_role`` in cli_config.json.
    ...
    *  ~~~
        $ cd xxxxxxx/ask_amy_example_skills/alexa_05_scorekeeper_skill
        $ vim cli_config.json
        ~~~
...
3. Create a new skill in your Amazon Development Account.
    ...
    * Enter a _Skill Name_ (scorekeeper) and select _custom_ as the skill type and _Provision your own_ as the method to host the backend. Then select `Create a skill`.
