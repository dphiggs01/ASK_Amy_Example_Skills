...
1. Update ``arn:aws:iam::**********:role/alexa_lambda_role`` in cli_config.json.
    ...
    *  ~~~
        $ cd xxxxxxx/ask_amy_example_skills/alexa_08_wise_guy_skill
        $ vim cli_config.json
        ~~~
...
3. Create a new skill in your Amazon Development Account.
    ...
    * Enter a _Skill Name_ (wise guy) and select _custom_ as the skill type and _Provision your own_ as the method to host the backend. Then select `Create a skill`.
